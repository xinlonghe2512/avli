...
from src.utils.santizer import (
    sanitize_email,
    sanitize_string,
...
async def register_user(_request: Request, user_data: UserCreate) -> UserResponse:
    ...
    try:
        ...
        sanitized_username = (
            sanitize_string(user_data.username) if user_data.username else None
        )
...
async def create_session(user: User = Depends(get_current_user)) -> SessionResponse:
    ...
    try:
        ...
        logger.exception(
            "session_creation_validation_failed", error=str(ve), user_id=user.id
        )
        raise HTTPException(status_code=422, detail=str(ve))
...
async def update_session_name(
    ...
    try:
        ...
        logger.exception(
            "session_update_validation_failed", error=str(ve), session_id=session_id
        )
        raise HTTPException(status_code=422, detail=str(ve))
...
async def delete_session(
    ...
    try:
        ...
        logger.info(
            "session_deleted", session_id=session_id, user_id=current_session.user_id
        )
    except ValueError as ve:
        logger.exception(
            "session_deletion_validation_failed", error=str(ve), session_id=session_id
        )
        raise HTTPException(status_code=422, detail=str(ve))
...
async def get_user_sessions(
    ...
    try:
        ...
        logger.exception(
            "get_sessions_validation_failed", user_id=user.id, error=str(ve)
        )
        raise HTTPException(status_code=422, detail=str(ve))
